...
Return ONLY the JSON array, nothing else. No explanation, no markdown fences.
"""
...
def build_html(all_events):
    ...
    for i, loc in enumerate(LOCATIONS):
        ...
        if not events:
            cards = '<p class="no-events">No events found for this location this week. Check back next Monday!</p>'
        else:
            for e in events:
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    select {{
      ...
      font-size: 0.93rem; border: 1px solid #d1d5db;
      border-radius: 8px; background: #fff;
      color: #1a1a2e; cursor: pointer; margin-bottom: 24px;
    }}
    .card {{
    ...
    .card h3 {{ font-size: 1rem; margin-bottom: 6px; color: #1a1a2e; }}
